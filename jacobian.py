...
class ARM_FTM:
    ...
    def __init__(self, L1, L2, L3):
        ...
        self.SF_ROT     = lambda θ : np.array([[1, 0, 0], [0, np.cos(θ), np.sin(θ)], [0, -np.sin(θ), np.cos(θ)]])
        self.SA_ROT     = lambda θ : np.array([[np.cos(θ), 0, np.sin(θ)], [0, 1, 0], [-np.sin(θ), 0, np.cos(θ)]])
        self.SR_ROT     = lambda θ : np.array([[np.cos(θ), -np.sin(θ), 0], [np.sin(θ), np.cos(θ), 0], [0, 0, 1]])
        self.SR_TRANS   = np.array([[0], [0], [self.L1]])
        self.EF_ROT     = lambda θ : np.array([[1, 0, 0], [0, np.cos(θ), np.sin(θ)], [0, -np.sin(θ), np.cos(θ)]])
        self.ER_ROT     = lambda θ : np.array([[np.cos(θ), -np.sin(θ), 0], [np.sin(θ), np.cos(θ), 0], [0, 0, 1]])
        self.ER_TRANS   = np.array([[0], [0], [self.L2]])
        self.WA_ROT     = lambda θ : np.array([[np.cos(θ), 0, np.sin(θ)], [0, 1, 0], [-np.sin(θ), 0, np.cos(θ)]])
        self.WA_TRANS   = np.array([[0], [0], [self.L3]])
...
def main():
    ...
    theta_SA_ROT = 30 * np.pi / 180
    theta_SR_ROT = 20 * np.pi / 180
    ...
    transform_S_TRANS = np.concatenate((transform_SA2SR, transform_SR_TRANS), axis = 1)
    ...
    #print(end_transform)
    endpoint = np.matmul(transform_S, point.T)
